...
<pre><code>1.	安装 Homebrew（如果尚未安装）：/bin/bash -c &quot;$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)&quot;
2.	安装 openssl 和 libffi：brew install openssl libffi
3.	在 mlcLLM 环境中使用 Conda 安装 PySocks：conda install -c conda-forge pysocks
4.	验证安装：安装完成后，通过以下命令验证 PySocks 是否已成功安装：pip show PySocks
</code></pre>
</div></template>
